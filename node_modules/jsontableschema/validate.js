...
exports.default = function(schema) {
  var fieldNames = _lodash2.default.map(schema.fields || [],
    _lodash2.default.property('name'));

  return new Promise(function(resolve, reject) {
    (0, _isomorphicFetch2.default)(
      'http://schemas.datapackages.org/json-table-schema.json')
  ...
  function errors(values) {
    ...
    try {
      for (var _iterator = values[Symbol.iterator](), _step;
    ...
    if (schema.primaryKey) {
      ...
      if (_lodash2.default.isString(primaryKey)) {
        if (!_lodash2.default.includes(fieldNames, primaryKey)) {
          valid = false;
          errs.push('primary key ' + String(primaryKey) +
        ...
        _lodash2.default.each(primaryKey, function(pk) {
          if (!_lodash2.default.includes(fieldNames, pk)) {
            valid = false;
            errs.push(
              'primary key ' + String(pk) + ' must match schema field names');
          }
        });
      }
    }
      ...
      _lodash2.default.each(foreignKeys, function(fk) {
        if (_lodash2.default.isString(fk.fields)) {
          if (!_lodash2.default.includes(fieldNames, fk.fields)) {
            valid = false;
            errs.push('foreign key ' + String(fk.fields) +
                      ' must match schema field names');
          }

          if (!_lodash2.default.isString(fk.reference.fields)) {
            valid = false;
            errs.push('foreign key ' + String(fk.reference.fields) +
                      ' must be same type as ' + String(fk.fields));
          }
          ...
          _lodash2.default.each(fk.fields, function(field) {
            if (!_lodash2.default.includes(fieldNames, field)) {
              valid = false;
              errs.push('foreign key ' + String(field) +
                        ' must match schema field names');
            }
          });
