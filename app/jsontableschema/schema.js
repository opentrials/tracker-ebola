...
var Schema = function () {
  ...
  _createClass(Schema, [{
    ...
    value: function castValue(fieldName, value) {
      ...
      var skipConstraints = arguments.length <= 3 || arguments[3] === undefined ? true : arguments[3];
    ...
    value: function castRow(items) {
      ...
      if (headers.length !== items.length) {
        throw new Array('The number of items to convert does not match the ' + 'number of fields given in the schema');
      }
        ...
        try {
          ...
          if (!skipConstraints) {
            ...
            if (this.uniqueness && this.uniqueHeaders) {
              _constraints2.default.check_unique(fieldName, value, this.uniqueHeaders, this.uniqueness);
            }
          }
          result.push(value);
        } catch (e) {
          ...
          switch (e.name) {
            case 'UniqueConstraintsError':
              ...
              break;
            default:
              error = 'Wrong type for header: ' + String(headers[i]) + ' and value: ' + String(items[i]);
          }
          if (failFast === true) {
    ...
    value: function requiredHeaders() {
      return _lodash2.default.chain(this.descriptor.fields).filter(function (field) {
