...
var UniqueConstraintsError = function (_Error) {
  ...
  function UniqueConstraintsError(message) {
    ...
    var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(UniqueConstraintsError).call(this, message));
...
exports.default = {
  ...
  check_required: function check_required(name, value, required) {
    if (required === true) {
      if (_lodash2.default.isUndefined(value) || _utilities2.default.isNull(value) === true) {
  ...
  check_minLength: function check_minLength(name, value, minLength) {
    if (value.length < minLength) {
      throw new Error('The field \'' + String(name) + '\' must have a minimum length of ' + minLength);
    }
    return true;
  }
  ...
  check_maxLength: function check_maxLength(name, value, maxLength) {
    if (value.length > maxLength) {
      throw new Error('The field \'' + String(name) + '\' must have a maximum length of ' + maxLength);
    }
    return true;
  }
  ...
  check_minimum: function check_minimum(name, value, minimum) {
    ...
    if (result) {
      throw new Error('The field \'' + String(name) + '\' must not be less than ' + String(minimum));
    }
    return true;
  }
  ...
  check_maximum: function check_maximum(name, value, maximum) {
    ...
    if (result) {
      throw new Error('The field \'' + String(name) + '\' must not be more than ' + String(maximum));
    }
    return true;
  }
  /**
  ...
  check_pattern: function check_pattern(name, value, pattern) {
    ...
    if (!matches || matches.length === 0) {
      throw new Error('The value \'' + String(pattern) + '\' for field \'' + String(name) + '\' must match the pattern');
    }
    return true;
  },
  check_enum: function check_enum(name, value, enumerator) {
    ...
    throw new Error('The value for field \'' + String(name) + '\' must be in the enum array');
  }
  ...
  check_unique: function check_unique(fieldName, value, headers, unique) {
    ...
    if (!unique.hasOwnProperty(fieldName)) {
      ...
      if (_lodash2.default.includes(unique[fieldName], value)) {
        throw new UniqueConstraintsError('Unique constraint violation for field name \'' + String(fieldName) + '\'');
      }
